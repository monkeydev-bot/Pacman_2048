...
perso = pygame.image.load("perso2.png").convert_alpha()
...
def gameplay_graphismes(n,value_max):
...
while continuer:
    ...
    for event in pygame.event.get():   #On parcours la liste de tous les événements reçus
        ...
        if event.type == KEYDOWN:
            if event.key == K_UP:
                command='up'
                position_perso=position_perso.move(0,-96)
                pygame.display.flip()
            if event.key == K_DOWN:
                command='down'
                position_perso = position_perso.move(0,96)
            ...
            if event.key == K_RIGHT:
                position_perso=position_perso.move(128,0)
                command='right'
            if event.key == K_LEFT:
                position_perso=position_perso.move(-128,0)
                command='left'
